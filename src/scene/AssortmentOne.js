...
export default class AssortmentOne extends React.Component {
    ...
    _getData=()=>{
        let parpam="thetype=1017&classtype=classify2&searchstr="+this.props.searchstr;
        Request('1017',parpam)
            .then((responseJson) => {
                    ...
                    <List style={{backgroundColor:'#fff'}}>
                        {this.state.data.map((item,i)=>
                            <ListItem key={i} button={true}
                                      onPress={()=>
                                      Actions.searchvideo({isassort:true,value:item.name,classify2:this.props.title,classify3:item.name})
                                      }
